...
const mNames = princesses.filter(princess => {if((princess.name).startsWith("M")){return true}});
...
let sum = 0;
const ageAve = princesses.reduce((total, princess) => {return (total+princess.age)}, 0)/princesses.length;
console.log(ageAve)
// for (let i = 0; i < princesses.length; i++) {
...
// const lNames = [];
const lNames = princesses.filter(princess => {if((princess.name).startsWith("L")){return true}})
console.log(lNames)
const ave = lNames.reduce((total, princess) => {return(total+princess.age)},0)
console.log(ave/lNames.length)
