...
function displayResults(data) {
  ...
  data.forEach(animal => {
    var newTr = $("<tr>")
    var newTd = $("<td>").text(`${animal.name}`);
    var newTd2 = $("<td>").text(`${animal.numLegs}`);
    var newTd3 = $("<td>").text(`${animal.class}`);
    var newTd4 = $("<td>").text(`${animal.weight}`);
    var newTd5 = $("<td>").text(`${animal.whatIWouldReallyCallIt}`);
    
    newTr.append(newTd, newTd2, newTd3, newTd4, newTd5); 
